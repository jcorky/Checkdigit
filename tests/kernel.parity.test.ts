...
describe("kernel-truth vectors (run_pass11.write_js_vectors)", () => {
  ...
  for (const v of vectors) {
    it(`explain(${JSON.stringify(v.input)})`, () => {
      const r = explain(v.input);
      expect(r.ok).toBe(v.ok);
      if (!r.ok) return;
      expect(r.kind).toBe(v.kind);
      expect(r.computed).toBe(v.computed);
      expect(r.verdict).toBe(v.verdict);
      expect(r.sum).toBe(v.sum);
      expect(r.full).toBe(v.full);
      if (r.kind === "iso6346_ilu") {
        expect(r.mod).toBe(v.mod);
        expect(r.category_set).toBe(v.category_set);
      }
    });
  }
});
...
function policyFromDict(d: PolicyDict): PolicyLike {
  ...
  return {
    resolve(prefix: string): ResolvedTreatment {
      ...
      if (hit !== null) {
        return { owner_policy: d.per_prefix[hit] as OwnerPolicy, force_flag: false, corroborated: false };
      }
...
describe("decision-table vectors (scripts/gen_decision_vectors.py)", () => {
    ...
    it(`${group} (${cases.length} cases)`, () => {
      for (const c of cases) {
        ...
        if (c.fn === "correct_identifier") {
          ...
          actual = correctIdentifier(c.token as string, c.context as FieldContext, {
            known_owner_prefixes: known,
            owner_policy: c.owner_policy as string,
            policy,
          });
          label = `${JSON.stringify(c.token)} ctx=${c.context} owner_policy=${c.owner_policy} known=${JSON.stringify(c.known_owner_prefixes)}`;
        } else {
          actual = correctX12Equipment(c.initial as string, c.number as string, c.printed_check ?? null, {
            policy,
          });
          label = `x12 ${c.initial}/${c.number}/${JSON.stringify(c.printed_check)}`;
        }
  ...
  it("raises the kernel's ValueError messages", () => {
    const fns: Record<string, (arg: string) => unknown> = {
      iso6346_value: (a) => iso6346CheckDigit(a.padEnd(10, "A").slice(0, 10)),
      iso6346_check_digit: iso6346CheckDigit,
      uic_check_digit: uicCheckDigit,
      correct_identifier: () => correctIdentifier("CSQU3054383", FieldContext.UNKNOWN, { owner_policy: "loose" }),
    };
    for (const e of data.errors) {
      const fn = fns[e.fn];
      if (!fn) throw new Error(`no mapping for ${e.fn}`);
      if (e.fn === "iso6346_value") {
        // exercise iso6346Value through a body that contains the bad character
        expect(() => iso6346CheckDigit(`${e.arg}PLU123456`.slice(0, 10)), e.arg).toThrow(e.message);
        continue;
      }
      expect(() => fn(e.arg), `${e.fn}(${e.arg})`).toThrow(e.message);
    }
  });
});
