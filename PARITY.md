...
|---|---|---|---|---|
| `_ISO6346_LETTER_VALUES` | 51-60 | `src/lib/checkdigit.ts` letter table | `tests/kernel.parity.test.ts` | OPEN |
| `iso6346_value` | 63-70 | `iso6346Value` | `tests/kernel.parity.test.ts` | OPEN |
| `iso6346_check_digit` | 73-92 | `iso6346CheckDigit` | `tests/kernel.parity.test.ts` | OPEN |
| `uic_check_digit` | 95-114 | `uicCheckDigit` | `tests/kernel.parity.test.ts` | OPEN |
| `IdentifierType` | 121-126 | `IdentifierType` (`ISO6346`, `ILU`, `UIC`, `SIZE_TYPE`, `UNKNOWN`) | `tests/kernel.parity.test.ts` | OPEN |
| `FieldContext` | 129-146 | `FieldContext` | `tests/kernel.parity.test.ts` | OPEN |
| `Status` | 149-154 | `Status` (`VALID`, `CORRECTED`, `FLAGGED`, `NOT_A_TARGET`, `INVALID_STRUCTURE`) | `tests/kernel.parity.test.ts` | OPEN |
| `CorrectionResult` | 157-171 | `CorrectionResult` | `tests/kernel.parity.test.ts` | OPEN |
| `_RE_BIC_LIKE` | 176 | `RE_BIC_LIKE` | `tests/kernel.parity.test.ts` | OPEN |
| `_RE_CONTAINER_SHAPED` | 177 | `RE_CONTAINER_SHAPED` | `tests/kernel.parity.test.ts` | OPEN |
| `_RE_UIC` | 178 | `RE_UIC` | `tests/kernel.parity.test.ts` | OPEN |
| `_ISO6346_CATEGORIES` (`UJZ`) | 179 | `ISO6346_CATEGORIES` | `tests/kernel.parity.test.ts` | OPEN |
| `_ILU_CATEGORIES` (`ABDEK`) | 180 | `ILU_CATEGORIES` | `tests/kernel.parity.test.ts` | OPEN |
| `_OWNER_POLICIES` | 181 | `OWNER_POLICIES` | `tests/kernel.parity.test.ts` | OPEN |
| `normalize` | 188-194 | `normalize` | `tests/kernel.parity.test.ts` | OPEN |
| `_evaluate_bic` | 197-208 | `evaluateBic` | `tests/kernel.parity.test.ts` | OPEN |
| `correct_identifier` decision table | 211-336 | `correctIdentifier` | `tests/kernel.parity.test.ts` | OPEN |
| `correct_identifier` policy layer (deny / allow / per-prefix) | 238-258 | `correctIdentifier` with `policy` option | `tests/kernel.parity.test.ts` | OPEN |
| `_decide_bic` | 339-384 | `decideBic` | `tests/kernel.parity.test.ts` | OPEN |
| `_decide_uic` | 387-410 | `decideUic` | `tests/kernel.parity.test.ts` | OPEN |
| `correct_x12_equipment` | 417-495 | `correctX12Equipment` | `tests/kernel.parity.test.ts` | OPEN |
| `explain` | 522-581 | `explain` | `tests/kernel.parity.test.ts` (vectors) | OPEN |

## Kernel test cases (`checkdigit/test_equipment_checkdigit.py`, 22 tests)

| Python test | TypeScript test | Status |
|---|---|---|
| `test_iso6346_canonical_example` (`CSQU305438` gives 3) | `tests/kernel.parity.test.ts` | OPEN |
| `test_iso6346_remainder_10_maps_to_zero` (`APLU100000` gives 0) | `tests/kernel.parity.test.ts` | OPEN |
| `test_uic_worked_example` (`21812471217` gives 3) | `tests/kernel.parity.test.ts` | OPEN |
| `test_iso6346_value_rejects_bad_char` | `tests/kernel.parity.test.ts` | OPEN |
| `test_snx_fixture` | `tests/kernel.parity.test.ts` | OPEN |
| `test_snx_valid_one_is_not_changed` | `tests/kernel.parity.test.ts` | OPEN |
| `test_freetext_checkfail_is_flagged_not_corrected` | `tests/kernel.parity.test.ts` | OPEN |
| `test_freetext_corroborated_by_owner_registry_is_corrected` | `tests/kernel.parity.test.ts` | OPEN |
| `test_unusual_category_is_flagged` | `tests/kernel.parity.test.ts` | OPEN |
| `test_ilu_category_routes_and_shares_math` | `tests/kernel.parity.test.ts` | OPEN |
| `test_size_type_passthrough` | `tests/kernel.parity.test.ts` | OPEN |
| `test_uic_requires_rail_context_to_correct` | `tests/kernel.parity.test.ts` | OPEN |
| `test_uic_correction_in_rail_context` | `tests/kernel.parity.test.ts` | OPEN |
| `test_garbage_is_invalid_structure` | `tests/kernel.parity.test.ts` | OPEN |
| `test_painted_form_is_normalized` | `tests/kernel.parity.test.ts` | OPEN |
| `test_x12_populates_absent_check_digit` | `tests/kernel.parity.test.ts` | OPEN |
| `test_x12_validates_correct_check_digit` | `tests/kernel.parity.test.ts` | OPEN |
| `test_x12_corrects_wrong_check_digit` | `tests/kernel.parity.test.ts` | OPEN |
| `test_x12_rejects_bad_initial` | `tests/kernel.parity.test.ts` | OPEN |
| `test_pseudo_prefix_strict_is_flagged` | `tests/kernel.parity.test.ts` | OPEN |
| `test_pseudo_prefix_lenient_is_corrected` | `tests/kernel.parity.test.ts` | OPEN |
| `test_standard_owner_unaffected_by_policy` | `tests/kernel.parity.test.ts` | OPEN |

## Kernel vectors and explain assertions (`checkdigit/run_pass11.py`)

| Python source | TypeScript test | Status |
|---|---|---|
| `write_js_vectors()`: 11 vectors in `tests/vectors/calc_vectors.json` | `tests/kernel.parity.test.ts` | OPEN |
| `test_explain_iso` per-character assertions (`chars[0]`, `chars[3]`, `remainder_ten`, `mod`) | `tests/kernel.parity.test.ts` | OPEN |
| `test_explain_uic_and_errors` (`steps` sum equals `sum`; malformed shapes return `ok: false`) | `tests/kernel.parity.test.ts` | OPEN |
| Required extra cases: `MSKU1234567` mismatch with computed 5; lowercase and space-separated token; unknown category gives FLAGGED; 12-digit token outside rail context gives FLAGGED even when Luhn passes | `tests/kernel.parity.test.ts` | OPEN |

## Existing JavaScript mirrors (to be consolidated into `src/lib/checkdigit.ts`)

| Source block | Replacement | Proven by | Status |
|---|---|---|---|
| `checkdigit/checkdigit_app.jsx:69-119` `calcExplain` | `explain` | `tests/kernel.parity.test.ts`, `tests/site.test.ts` (CALC-PURE equality) | OPEN |
